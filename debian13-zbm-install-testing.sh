...
# Verify downloads against the release sha256.txt. Aborts on mismatch.
ZBM_VERIFY="${ZBM_VERIFY:-yes}"
...
# Empty = every feature flag the installing ZFS supports. ZBM v3.1.0 images
# carry OpenZFS 2.4.0, so they can import a 2.4-upgraded pool.
...
ENABLE_BACKPORTS="${ENABLE_BACKPORTS:-yes}"   # low priority, opt-in per package
...
ENABLE_PROPOSED_UPDATES="${ENABLE_PROPOSED_UPDATES:-yes}"
...
SHAFILE=/root/zbm-sha256.txt
if [[ "$ZBM_VERIFY" == "yes" ]]; then
	if curl -fSL --retry 3 -o "$SHAFILE" "${ZBM_BASE_URL}/${ZBM_VERSION}/sha256.txt"; then
		echo "  Fetched sha256.txt for verification."
	else
		warn "Could not fetch sha256.txt; downloads will NOT be verified."
		: > "$SHAFILE"
	fi
else
	: > "$SHAFILE"
...
fetch_zbm() {
	...
	if [[ ! -f "$tmp" ]]; then
		...
		if [[ -s "$SHAFILE" ]]; then
			local want got
			want="$(awk -v f="$fname" '$2 == f || $2 == "*" f { print $1; exit }' "$SHAFILE")"
			if [[ -n "$want" ]]; then
				...
				warn "No checksum listed for ${fname}; not verified."
			fi
		fi
	fi
...
rm -f /root/zfsbootmenu-*.EFI "$SHAFILE"
...
APT SUITES ENABLED
  ...
  Components: ${COMPONENTS}
$( [[ "$ENABLE_PROPOSED_UPDATES" == yes ]] && printf '%s\n' \
"  proposed-updates has NORMAL priority: 'apt upgrade' will pull from it." \
"  Comment it out in /etc/apt/sources.list if you want stable behaviour." )
  Backports is low priority; use it explicitly:
